<?php 
    ...
    if(isset($_GET['status']) && isset($_GET['requestid'])){
        ...
        {
            $ck = $ĐGB->get_row("SELECT * FROM `".myGroupExCard($getUser['username'])."` WHERE `loaithe` = '$telco' AND `menhgia` = '$value'  ")['ck'];
    ...
    if($txtBody || $jsonBody)
    {
        ...
        if($status == true)
        {
            ...
            if($value != $declared_value)
            {
                ...
                $ck = $ĐGB->get_row("SELECT * FROM `".myGroupExCard($getUser['username'])."` WHERE `loaithe` = '$telco' AND `menhgia` = '$menhgia'  ")['ck'];
